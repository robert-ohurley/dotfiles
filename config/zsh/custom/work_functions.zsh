...
goldfinger_to_staging() {
  ...
  # If it's an IP (with or without port), connect first.
  if [[ "$target" =~ ^[0-9]+\.[0-9]+\.[0-9]+(:[0-9]+)?$ ]]; then
    adb connect "$target" >/dev/null 2>&1 || true
  fi

  # Try root if supported (no-op on non-rooted/user builds).
...
goldfinger_to_prod() {
  ...
  # Try root if supported (no-op on non-rooted builds).
  adb -s "$target" root >/dev/null 2>&1 || true

  # Run commands inside the device
  adb -s "$target" shell <<'EOF'
set -e
